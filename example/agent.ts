import * as fs from 'fs';
import Web3 = require('web3');
import { ABIDefinition  } from 'web3/eth/abi';
import { BigNumber } from 'bignumber.js';
import EthTx = require("ethereumjs-tx");
import { Agent } from '../index';

type Resource = {abi: ABIDefinition[], bytecode: string}

const testAddr = '0x345ca3e014aaf5dca488057592ee47305d9b3e10';
const testResource: Resource = JSON.parse(fs.readFileSync('./build/contracts/Test.json', 'utf-8'));

const settings = {
    gasPrice: '100000000000',
    gasLimit: '6000000',
}

function makeTx (nonce: number, senderKey: Buffer, to: string, value: number|string, data: string) {
    let tx = new EthTx({
        to: to === null ? undefined : to,
        data: data,
        nonce: nonce,
        value: '0x' + new BigNumber(value).toString(16),
        gasPrice: '0x' + new BigNumber(settings.gasPrice).toString(16),
        gasLimit: '0x' + new BigNumber(settings.gasLimit).toString(16),
    });
    tx.sign(senderKey);
    return tx;
}

const agent = new Agent(() => new Web3.providers.WebsocketProvider('ws://localhost:9545'));

agent.setAction('nonce', async ([address]: [string]) => {
    return {
        error: null,
        data: await agent.web3.eth.getTransactionCount(address),
    }
});

agent.on(
    testAddr,
    testResource.abi.filter(abi => abi.type === 'event' && abi.name === 'Add')[0],
    data => {
        agent.emit('add', data.value);
    });

agent.setAction('add', async ([value]: [number]) => {
    const sender = '0x627306090abab3a6e1400e9345bc60c78a8bef57';
    const sk = 'c87509a1c067bbde78beb793e6fa76530b6382a4c0241e5e4a9ec0a0f44dc0d3';
    const nonce = await agent.web3.eth.getTransactionCount(sender);
    const test = new agent.web3.eth.Contract(testResource.abi, testAddr);
    const data = test.methods.add(value).encodeABI();
    const tx = makeTx(nonce, Buffer.from(sk, 'hex'), testAddr, 0, data);
    await agent.web3.eth.sendSignedTransaction(`0x${tx.serialize().toString('hex')}`);
    return {
        error: null,
        data: value,
    };
});

agent.serve(8080, '');
